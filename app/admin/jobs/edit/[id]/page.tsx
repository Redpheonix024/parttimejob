...
export default function EditJob() {
  ...
  // Add state to track loading job data
      ...
      try {
        ...
        if (jobSnap.exists()) {
          ...
          setFormData({
            ...
            gender: job.gender || "",
            minAge: job.ageRange?.min?.toString() || "18",
            maxAge: job.ageRange?.max?.toString() || "",
            ...
            termsAccepted: false, // Don't pre-check terms
            address: job.location?.address || "",
            city: job.location?.city || "",
            state: job.location?.state || "",
            zip: job.location?.zip || "",
            buildingName: job.location?.buildingName || "",
            googleMapsLink: job.location?.customGoogleMapsLink || "",
          });
          setLocation({
            lat: job.location?.coordinates?.lat || 10.52753,
            lng: job.location?.coordinates?.lng || 76.214514,
          });
          setWorkLocation(job.workLocation || "on-site");
          setContacts(job.contacts || [{ name: "", phone: "" }]);
          // If audio exists, set audioUrl (for playback)
          if (job.audioDescription?.url) {
            setAudioUrl(job.audioDescription.url);
            setRecordingTime(job.audioDescription.duration || 0);
          }
        } else {
  ...
  // Update handleSubmit to update existing job
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (workLocation === "on-site") {
      const hasGoogleMapsLink =
        formData.googleMapsLink && formData.googleMapsLink.trim() !== "";
      ...
      if (
        ...
        errors.googleMapsLink =
          "Either address details or a Google Maps link is required";
      }
    }
    ...
    try {
      ...
      // Prepare job data (do not generate new jobId)
      const jobData: any = {
        // id: jobId, // id is already the doc id
        userId: user.uid,
      ...
      if (workLocation === "on-site") {
        ...
        const locationData: any = {
          ...
          customGoogleMapsLink: hasGoogleMapsLink
      ...
      // Upload audio to S3 if exists and changed
      if (audioBlob) {
        try {
          ...
          toast.error(
            "Failed to upload audio. Job will be updated without audio."
          );
        }
      } else if (audioUrl) {
        // If audioUrl exists and no new blob, keep existing audioDescription
        // (do nothing, Firestore merge will keep it)
      } else {
        jobData.audioDescription = null;
      }
  ...
  return (
    <AdminLayout activeLink="jobs" title="Update Job">
      {/* Google Maps Script */}
      <Script
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-muted-foreground mb-8">
            Update the details of your part-time job opportunity.
          </p>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              setShowGoogleMapsLink(!showGoogleMapsLink)
                            }
                            className="text-xs"
                          >
